...
function EngageProductSelect({ options, disabled, defaultValue, onChange, required, title }) {
	...
	return (
		<FormGroup>
			...
			<ListProducts disabled={disabled}>
				{_.map(filterOptions(), option => (
					<Product key={option.id} onClick={() => onSelect(option)}>
						<span>{option.name}</span>
...
export default EngageProductSelect;
